...
This project is not just a single tool panel. It is a configurable toolchain platform for the SillyTavern / TavernHelper host environment. The main capabilities described in the repo docs are:
- API configuration and preset management
- custom tool management
- regex/rule-based extraction
- automatic trigger-and-run behavior after host generation events
...
- `settings-service.js` — cached global settings for executor, listener, debug, and UI groups
...
- runtime status/diagnostic fields used by the UI and trigger pipeline
...
The main automatic tool chain is:

`tool-trigger -> tool-output-service -> tool-prompt-service -> api-connection -> context-injector`

Responsibilities:
- `modules/tool-trigger.js` listens to SillyTavern/TavernHelper events, applies gating/dedup/session logic, and decides whether a tool should run
...
For bugs around “did not trigger”, “triggered twice”, or “writeback failed”, inspect this chain in order.

### Output modes

Tool execution behavior is organized around two output modes in `modules/tool-output-service.js`:
- `follow_ai` — follows the main AI output path without the extra post-response parsing chain
...
- `modules/ui-components.js` — compatibility facade; prefer `modules/ui/index.js` for new work
...
- `modules/tool-executor.js` still exists for older execution paths and generic scheduling, but the main automatic chain is centered on `tool-trigger` + `tool-output-service`
...
This plugin is designed to run inside SillyTavern / TavernHelper and relies on host-provided runtime capabilities. The code expects browser/host globals and host event semantics rather than a standalone Node app architecture.

That matters for:
- event handling in `tool-trigger.js`
- API dispatch behavior in `api-connection.js`
- UI integration and popup mounting
- writeback behavior in `context-injector.js`

Be careful not to “simplify” host-specific logic that exists to handle quiet generation, dry-run generation, rerolls/swipes, or host-side writeback quirks.

## Important docs to consult

- `README.md` — current product-level overview and module positioning
- `docs/ARCHITECTURE_ANALYSIS.md` — big-picture architecture and the old-vs-new layering context
- `docs/HOST_REGRESSION_CHECKLIST.md` — authoritative manual regression checklist for trigger/writeback behavior
- `docs/API_DOCUMENTATION.md` — public API surface and usage notes
- `docs/OPTIMIZATION_PROGRESS.md` — recent structure-convergence notes
...
- For trigger/writeback bugs, validate against the manual host regression checklist, not just a successful bundle.
