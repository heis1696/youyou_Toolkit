...
- 从“大表单”推进到“侧栏表列表 + 当前表主编辑区 + 辅助区”的 visualizer MVP 壳层
- 表列表承载当前激活表、顺序调整与轻量状态提示
- 列顺序调整
- 行顺序调整
- 更成熟的单元格编辑体验
- 更适合大表的交互布局
- 把校验摘要、编译预览、保存态提示、高级 authoring 项从主编辑流里拆成更清晰的辅助区或折叠区
...
- 更清晰的运行结果、目标信息、加载信息分块展示
- 更好的错误提示与校验反馈
- 高级诊断面板

### 4.4 Template & History
...
1. **先补 Table UI**
   - 先把 workbench 推进为 visualizer MVP 壳层
   - 再补表 / 行 / 列排序与更顺手的编辑体验
2. **再补 Authoring Model**
   - draft / template / compiled 分层
   - 更明确的列定义元信息
3. **再补 Runtime / Diagnostics**
   - 强化运行结果与诊断展示
4. **最后再决定是否进入 Template & History / Automation**

原因很简单：
- 当前执行链已经稳定
- 当前 authoring 已从 JSON 过渡到结构化 MVP
- 现在最缺的不是“能不能跑”，而是“能不能把编辑器做成真正可长期使用的工作台”
- 参考项目进一步说明：当前最值得补的不是新执行层，而是“工作台焦点 + 渐进展开 + 主辅分区”的 UI 壳层
...
> **`tableWorkbench` 已经从 JSON-only 的最小手动填表 MVP，推进到了带结构化表定义编辑器的 MVP；执行主链稳定，但 Table UI、Authoring Model、Template & History 与 Automation 仍未完成。**