...
- **Ai 指令预设消息排序改为按钮** (`modules/ui/components/bypass-panel.js`)
  - 将 1.0.125 的拖拽排序改为上移/下移/插入按钮，移除拖拽实现

## [1.0.125] - 2026-05-09

### 新增

- **Ai 指令预设消息拖动排序与插入** (`modules/ui/components/bypass-panel.js`)
  - 预设消息列表支持拖动排序
  - 支持在任意位置插入新消息
...
- ✨ **世界书注入过滤禁用条目** (`modules/table-engine/table-worldbook-sync-service.js`, `modules/table-engine/table-writeback-service.js`, `modules/table-engine/table-schema-service.js`, `modules/ui/components/table-workbench-panel.js`)
  ...
  - 手动和自动填表都会触发同步
- **多世界书分隔标签** (`modules/tool-worldbook-service.js`)
  - 多世界书内容拼接时使用 `\n\n---\n\n` 分隔，增强可读性
...
- 🐛 **修复 bypass 无 mainSlot 时模板内容底部重复追加** (`modules/tool-prompt-service.js`)
  - 当 bypass 消息中无 mainSlot 角色时，模板内容不再重复追加到末尾
