...
- **词条级世界书 override UI**（#5）：自定义模式世界书预设中，每本世界书可展开词条列表，对单条目启用/禁用 override。注入时读取 `book.entryOverrides` 过滤。支持搜索过滤、override 计数显示、逐条清除 override
- **写回正文标签编辑**（#2）：数据编辑器全局注入模式新增 `mirrorTag` 输入框，保存到全局时同步写入 workbench config

### 修复

- **预设 sub-tab 切换不清空选中**（#1）：跨 kind 切换预设面板时保留 store 记录的当前选中，不再重置为空

### 优化

- **Hero chips 折叠**（#3）：工作台 hero 区域 chip 行默认单行显示（`max-height: 24px`），溢出隐藏，末尾 `▸/▾` 按钮点击展开/收起
