...
- 🎨 **破限词面板组件重构** (`modules/ui/components/bypass-panel.js`) - 重大UI改进
  - **移除二级编辑窗口**：删除了弹出式编辑对话框，改为一级窗口直接编辑，解决内容显示不全问题
  - **左右分栏布局**：左侧为预设列表，右侧为编辑区域，布局更加清晰
  - **高对比度按钮样式**：新增 `.yyt-btn-accent` 和 `.yyt-btn-outline` 按钮样式，按钮与背景区分明显
  - **实时编辑**：点击预设列表中的编辑按钮直接在右侧编辑区域编辑，无需弹出窗口
  - **角色颜色标识**：消息段落左侧边框根据角色类型显示不同颜色（SYSTEM红色、USER蓝色、assistant绿色）
  - **状态指示器**：预设列表显示当前默认预设（蓝色）和正在编辑的预设（绿色）状态
  - **响应式布局**：支持移动端自适应，小屏幕自动切换为上下布局

### 移除

- 🗑️ 移除了 `_showBypassEditDialog` 方法和相关二级对话框代码
- 🗑️ 移除了对 `BypassEditor` 组件的依赖（功能已整合到主面板）
