...
> 更新时间：2026-03-24  
> 分析基线：2026-03-21 的 v0.6.2 + Unreleased 起点（本文主体保留施工前基线分析，并已追加施工后复核摘要）  
> 分析范围：`index.js`、`modules/`、`modules/ui/`、`docs/`

> 说明（2026-03-24 复核）：本文主体内容保留的是本轮优化启动前的“问题基线分析”，用于解释为什么需要 Phase 1 ~ Phase 5 这轮收敛。因此文中诸如“`index.js` 仍然过重”“UI 基础设施未完全统一”等表述，应理解为**施工前判断**，不应直接当成当前最终代码现状。当前落地状态请结合 `docs/OPTIMIZATION_PROGRESS.md` 与本文末尾“施工后复核摘要”一起阅读。
...
就 2026-03-21 的施工前基线而言，可以用一句话总结：

**入口仍重、服务层已成型、工具平台化已跑通、兼容层仍在过渡。**

## 9.3 当前最值得关注的后续工作

对于当时的施工前基线，最值得优先收敛的是三点：
...
## 十二、施工后复核摘要（2026-03-24）

为避免把上面的基线分析误读成当前现状，这里补充一份施工完成后的简要复核。

### 12.1 基线问题与当前状态对照

| 基线问题 | 当前状态 |
|------|------|
| `index.js` 仍然过重 | **已解决**。当前入口已收敛为薄入口，初始化、弹窗壳与对外 API 已分别下沉到 `modules/app/bootstrap.js`、`modules/app/popup-shell.js`、`modules/app/public-api.js` |
| 旧工具模型 / 新工具模型并存导致语义漂移 | **已大幅收敛**。当前运行主模型已统一到 `tool-registry.js`，`tool-manager.js` 主要承担定义层与兼容归一化入口 |
| `tool-executor.js` 同时承担主链和历史职责 | **已解决**。当前自动主链已明确为 `tool-trigger -> tool-output-service -> tool-prompt-service -> api-connection -> context-injector`，`tool-executor.js` 仅保留调度与兼容执行入口 |
| UI 装配中心未统一 | **已大幅解决**。`modules/ui/index.js` 现为 UI 主装配入口，`ui-components.js` 已降级为 compatibility facade |
| 自动链缺少足够调试信息 | **已解决**。当前已补齐 `lastAutoTriggerSnapshot`、单工具运行态诊断字段与折叠式诊断展示 |
| 运行时强依赖宿主全局对象 | **仍然存在**，但不属于本轮优化范围，继续作为宿主环境耦合现实约束保留 |
| 事件量增长后的治理压力 | **仍需长期关注**，但当前尚未构成本轮 Phase 的阻塞项 |

### 12.2 施工后的当前架构判断

如果用一句话描述当前状态，可以更新为：

**入口已瘦身、运行主模型已统一、自动主链已清晰、UI 主路径已收口、诊断能力已补齐，但系统仍然继续依赖宿主环境与真实运行时序。**

### 12.3 复核结论

因此，这份文档现在更适合作为：

1. **施工前问题基线档案**：解释为什么当初需要启动本轮优化
2. **施工后对照参照物**：帮助确认哪些问题已经被收敛，哪些仍然属于后续长期议题

若要查看“现在做到哪一步”，应优先看 `docs/OPTIMIZATION_PROGRESS.md`；若要查看“原计划是什么”，应优先看 `docs/OPTIMIZATION_EXECUTION_PLAN.md`。
