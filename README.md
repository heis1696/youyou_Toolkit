...
- 填表工作台视觉重做为更接近数据库工作台的层级：运行控制台卡片、active 表格导航卡、手动更新 checkbox-card、单表编辑抽屉、字段卡片、数据行卡片和诊断折叠区统一使用 scoped dark card language
- 填表工作台提供内置默认剧情状态模板，支持显式应用模板，可解析 shujuku 风格 `sheet_*` 模板，并会把旧版空占位“表格 1”自动迁移为默认 8 张表，同时复用破限模块的 Ai 指令预设作为填表请求前置消息
- 兼容模块按需加载，降低启动期负担
