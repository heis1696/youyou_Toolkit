...
> 当前文档以仓库内现行代码为准；当前发布版本为 `1.0.100`。
...
- 填表工作台提供内置默认剧情状态模板，支持显式应用模板，可解析 shujuku 风格 `sheet_*` 模板；模板库、聊天 guide 与提示词预设分层保存，live table rows 仍只来自结构化提交状态
- 填表工作台支持导入 / 导出 shujuku 填表 prompt group，选择提示词预设后会按 segment 顺序构建填表请求；未选择预设时继续沿用 legacy `promptTemplate`
- 兼容模块按需加载，降低启动期负担
